...
void OARSteinerTree::Compute_Priority_CTF(Database &database,elmoreDelay* elmoreModel)
{
	...
	if(secondTime==false){
		...
		while(infile.good()){
			...
			int x = atoi(line.substr(index1+1, index2-index1-1).c_str());
			int y = atoi(line.substr(index2+2, index3-index2-2).c_str());
			string delay_s = line.substr(index4+3,-1);
			istringstream delay_ss(delay_s);
			double delay_f;
			delay_ss>>delay_f;
			cout<<x<<"\t"<<y<<"\t"<<delay_f<<endl;
			int delay = static_cast<int>(delay_f);
			vector<int> innode (3);
			innode.at(0)=x;
			innode.at(1)=y;
			innode.at(2)=delay;
			  ...
			  for (int j =0; j<delays.size();j++){
				  ...
				  if((delays.at(j).at(0)==N->getX()) && (delays.at(j).at(1)==N->getY())){
					  priority=N->getRequiredTime()-delays.at(j).at(2);
				  	  cout<<N->getRequiredTime()<<"\t"<<delays.at(j).at(2)<<"\t"<<priority<<endl;
				  }
			  }
