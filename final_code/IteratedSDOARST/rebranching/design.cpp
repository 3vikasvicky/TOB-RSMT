...
void Design::ReadBranchFile (){
    ...
    while(!feof(fp)) {
        ...
        *line = '\0';
		...
		/*
		if(root_flag==0){
			node.type_ = Node::ROOT;
		}
		else{
			node.type_ = Node::SINK;
		}
		*/
...
void Design::ReadBuffer (){//from small to big ones
    ...
    double ratio = 1;
    double input_capacitance = 1*ratio;
    double driving_resistance = 1*ratio;
    double intrinsic_delay = 1*ratio*ratio;
    double slew_resistance = 100*ratio;
    double intrinsic_slew = 200*ratio*ratio;
    double cost = 1;//*ratio??
...
set<int> Design::RecognizeSinks(vector<Node>& nodes)const{
	...
	*/
	...
	for (int j=0; j<nodes.size(); j++){
		...
		cout<<nodes[j].point_.x<<"\t"<<nodes[j].point_.y<<endl;
		while(!feof(fp)) {        
			*line = '\0';
			fgets(line, LINESIZE, fp);
			int x, y;
			sscanf(line, "%d %d\n", &x, &y);
			if(x==nodes[j].point_.x && y==nodes[j].point_.y){
				sink_set.insert(j);
				cout<<j<<endl;
...
void Design::PropagateCap(vector<Node> & nodes)const{
    for (int i = 1; i < nodes.size(); i++){
        ...
        if (node.type_ == Node::SINK)//If it is sink, beside path cap, sink cap should be included 
            cap += node.ct_;
        int node_index = i;
        do{
        ...
        }while (node.type_ != Node::BUFFER && node_index != 0);//Node itself is not buffer or root
...
void Design::PropagateElmore(double delay, int node_index, const vector<Node>& nodes, map<int,double>& slew_map, map<int,double>& delay_map)const{
    ...
    if((fp=fopen(file_name, "r"))==NULL) {
		...
		while(!feof(fp)) {        
			*line = '\0';
			fgets(line, LINESIZE, fp);
			int x, y, current_index, next_index;
			sscanf(line, "%d\t%d\t%d%d\n", &current_index, &x, &y, &next_index);
			if(x==node.point_.x && y==node.point_.y){
				sink_flag=1;
			}
	}
	fclose(fp);
    ...
    if (node.type_ != Node::SINK || !children.empty()){
		if(sink_flag){
        ...
        for (set<int>::const_iterator it = children.begin(); it != children.end(); it++){
            ...
            DTYPE distance = edge.Length();
            double child_in_cap = child_node.type_==Node::BUFFER?
...
void Design::CalculateElmore(const vector<Node>& nodes, map<int,double>&slew_map, map<int,double>&delay_map)const{//Calculate Elmore values for all EP and MP. EP and MP may have different size due to eliminating MP
    ...
    double root_delay = nodes[0].buffer_.get_driving_resistance()*nodes[0].ct_;
