...
public:
    ...
    /*--------------  Buffering functions      ------------------*/
    void ReadBuffer (vector<Node> & nodes);
    ...
    double Evaluate(double& tns, double& wns, double period, const map<int,double>& delay_map);
    ...
    void CalculateElmore (vector<Node>&, map<int,double>&, map<int,double>&)const;
};
#endif
