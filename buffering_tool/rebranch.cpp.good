...
double Design::PrintTiming(const vector<Node> nodes, double period){
    ...
    strcat(file_name, "/");
    strcat(file_name, "timing.txt");
	outfile.open(file_name);

	ofstream logfile;
	char fileName[BUFFERSIZE];
	strcpy(fileName, benchmark_path_);
    strcat(fileName, "/");
    ...
    for (map<int,double>::const_iterator it = elmore_map.begin(); it != elmore_map.end(); it++){
		...
		logfile<<"Delay at Node"<<it->first<<"("<<nodes.at(it->first).point_.x<<", "<<nodes.at(it->first).point_.y<<") is "<<it->second<<endl;
        if (period - it->second < wns){
            worst_sink = it->first;
            wns = period - it->second;
        }
